...
let deckA = [					//14 Karten Deck generieren
	{ ...Baldur },
	{ ...Chimäre },
	{ ...Frigg },
	{ ...Harpye },
...
let deckB = [					//19 Karten Deck generieren
	{ ...Aphrodite },
	{ ...Chimäre },
	{ ...Hades },
	{ ...Harpye },
...
function mischeDeck(deck) {									// Mischen der Decks 
...
function draw( deck, hand ) {								// Allgemeines ziehen zum zugbeginn
	if ( deck.length === 0 ) {
		return null ;
	}
	if ( hand.length >= 8 ) {
...
function handStart( deck, hand ) {							// Karten ziehen für die Hand 
	for( let i = 0; i < 5; i++) {	
		draw( deck, hand ) ;		
	}
}
// Chat GPT kopiert >>>>
function holeAktiveHand() {									// Wechsel Eigene Hand 
	if ( aktiverSpieler === 1 ) {
		return handP1 ;
	}
	return handP2 ;
...
function holeGegnerHand() {									// Wechsel Gegner Hand 
	if ( aktiverSpieler === 1 ) {
		return handP2 ;
	}
	return handP1 ;
...
function holeAktivesFeld() {								// Wechsel eigenes Feld 
	if ( aktiverSpieler === 1 ) {
		return feldP1 ;
	}
	return feldP2 ;
...
function holeGegnerFeld() {									// Wechsel Gegner Feld
	if ( aktiverSpieler === 1 ) {
		return feldP2 ;
	}
	return feldP1 ;
...
//<<<< Chat GPT kopiert
function bauHand( hand, feld ) {											// Karten werden erstellt und anklickbar
	feld.innerHTML = "" ;
...
function spieleKarte( index, hand, feld ) {								// spielen der Karten auf ein Feld beschränkt 
	if ( feld.length >= 1) {
		return ;
	}
...
// Chat GPT kopiert >>>>												// Button erstellung
function bauMenu() {
...
function render() {														// Funktionsaufruf aller Funktionen 
...
overlay.addEventListener( "click", function() {							// Wechselbildschirm
